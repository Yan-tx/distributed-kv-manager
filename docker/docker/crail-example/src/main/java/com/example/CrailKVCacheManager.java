...
public class CrailKVCacheManager {
    ...
    // 获取共享的Crail连接
    public static CrailStore getStore() throws Exception {
        synchronized (storeLock) {
            if (store == null) {
                System.err.println("Initializing new Crail connection");
                
                // 设置配置文件路径
                System.setProperty("crail.conf.dir", "/root/crail/conf");
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            switch (operation) {
                ...
                case "exists":
                    if (args.length < 2) {
                        System.err.println("Exists requires <crail_path>");
                        printUsage();
                        System.exit(1);
                    }
                    boolean exists = existsFile(storeInstance, args[1]);
                    System.out.println(exists ? "true" : "false");
                    System.exit(exists ? 0 : 1);
                    break;
     ...
     * 从标准输入流上传数据到Crail
     */
    public static void uploadFromStream(CrailStore store, String crailPath) throws Exception {
     ...
     * 将Crail中的数据下载到标准输出流
     */
    public static void downloadToStream(CrailStore store, String crailPath) throws Exception {
    ...
    public static void uploadFile(CrailStore store, String localPath, String crailPath) throws Exception {
    ...
    public static void downloadFile(CrailStore store, String crailPath, String localPath) throws Exception {
    ...
    public static void listDirectory(CrailStore store, String crailDirPath) throws Exception {
    ...
    public static void createParentDirectories(CrailStore store, String path) throws Exception {
    ...
    public static void printProgress(String action, long current, long total) {
...
// 新增: 检查文件是否存在
public static boolean existsFile(CrailStore store, String crailPath) throws Exception {
    try {
        // 查找文件节点
        CrailNode node = store.lookup(crailPath).get();
