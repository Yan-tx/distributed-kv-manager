...
if [[ ! -d "$hugepage_path" ]]; then
  echo "错误：系统未启用2MB HugePages支持，目录 $hugepage_path 不存在"
  exit 1
fi
...
if (( available_hugepages == 0 )); then
  echo "错误：当前系统没有可用的2MB HugePages，请先配置"
...
if [[ -z "$mountpoint_data" ]]; then
  ...
  mount -t hugetlbfs -o pagesize=2M nodev /mnt/hugepages/data
...
if [[ -z "$mountpoint_cache" ]]; then
  ...
  mount -t hugetlbfs -o pagesize=2M nodev /mnt/hugepages/cache
